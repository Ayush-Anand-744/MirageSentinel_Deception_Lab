#!/bin/bash
# MirageSentinel Honeypot Orchestrator - Main Setup Script
...
Vagrant.configure("2") do |config|
  ...
  config.vm.network "private_network",
    libvirt__network_name: "miragesentinel_internal",
    ip: "10.20.20.1",
    netmask: "255.255.255.0"
...
deploy_crdt() {
    ...
    local binary_path="${CRDT_DIR}/target/x86_64-unknown-linux-musl/release/mirage-crdt"
    ...
    for vm in $vms; do
        ...
        # Copy binary to VM
        if ! vagrant scp "$binary_path" /tmp/mirage-crdt 2>/dev/null; then
            # Fallback: use SSH cat
            if ! vagrant ssh -c "cat > /tmp/mirage-crdt" < "$binary_path" 2>/dev/null; then
        ...
        if vagrant ssh -c "
          sudo mv /tmp/mirage-crdt /usr/local/bin/syslogd-helper && \
...
main() {
    case "${1:-setup}" in
        "setup")
            ...
            log_success "MirageSentinel Honeypot Infrastructure setup complete!"
