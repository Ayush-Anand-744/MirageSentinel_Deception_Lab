...
fn main() {
    ...
    let mut state = MirageSentinelState::load(STATE_FILE, &node_id);
    ...
    match args.get(1).map(|s| s.as_str()) {
        ...
        Some("merge") => {
            if let Some(path) = args.get(2) {
                let remote = MirageSentinelState::load(path, &node_id);
...
fn run_daemon(mut state: MirageSentinelState) {
    ...
    loop {
        ...
        state = MirageSentinelState::load(STATE_FILE, &state.node_id);
