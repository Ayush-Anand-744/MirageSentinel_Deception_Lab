...
# MirageSentinel Deception Lab - Project Documentation

## Overview

**MirageSentinel Deception Lab** is an enterprise-grade cybersecurity deception platform that creates realistic honeypot infrastructures to detect, engage, and analyze cyber attackers. The system builds parallel "fake" networks that mirror real enterprise infrastructure, allowing security teams to observe attacker behavior in a controlled, monitored environment.
...
│   ├── mirage-crdt/             # CRDT synchronization binary
...
│
└── MirageSentinel.pdf, Synopsis Presentation.pptx  # Documentation
```
...
```
        ...
        ┌───────────────┴───────────────┐
        │                               │
   corp_net (real)                miragesentinel_net (fake)
...
- `miragesentinel_net` (10.20.20.0/24) - Deception fabric honeynet
...
virsh net-define simulations/miragesentinel_net.xml
...
- **Technical Details:** `MirageSentinel.pdf`
