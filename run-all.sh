...
MONGO_CONTAINER="miragesentinel-mongo"
MONGO_PORT="27017"
BACKEND_PORT="3001"
FRONTEND_PORT="3000"
SEED_DB="${SEED_DB:-0}"
...
ensure_networks() {
  ...
  ensure_network_defined "miragesentinel_net" "$ROOT_DIR/simulations/miragesentinel_net.xml"
}
...
start_backend() {
  ...
  if open_in_terminal "MirageSentinel Backend" "$backend_cmd"; then
...
start_frontend() {
  ...
  if open_in_terminal "MirageSentinel Frontend" "$frontend_cmd"; then
...
main() {
  log "Starting MirageSentinel stack from: $ROOT_DIR"
